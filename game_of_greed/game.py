...
class Game:
    ...
    """
    Description:
    This class repreasent the core of Game Of Greed which contains:

    Methods: 
    - play: Starts a new game with multiple options, such as enter, quit and bank which are depending on the user input.
    ...
    def play(self, roller=GameLogic.roll_dice):
        """
        Description: Starts a new game with multiple options, such as enter, quit and bank which are depending on the user input.
        Args:
    ...
    def bank(self):
        """
        Description: This function is responsible for banking, shelfing the points and gives a summary of the recorded points at the end of each round
        Args:
            None
        """
        print(
    ...
    def play_again(self):
        """
        Description: This function is responible for upstomizing the user input by removing the useless spacess, aslo, its calling the calcualte function to calculate the actual recorded points.
        Args:
            None
        """
        if self.round_num >= 20 or self.banker.balance >= 10000:
        ...
        while flag:
            ...
            if keep_q2 == "q" or keep_q2 == "quit":
                print(
                    f"Thanks for playing. You earned {self.banker.balance} points")
                quit()
            # if true, print cheater else continue to the rest of code
            elif self.detect_type_or_cheater(keep_q2, a):
                ...
                shelfed_points = self.banker.shelf(
                    self.new_game.calculate_score(earned_points))
                print(
                    f"You have {shelfed_points} unbanked points and {self.dices_left} dice remaining")
